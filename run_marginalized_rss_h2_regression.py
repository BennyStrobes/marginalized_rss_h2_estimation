...
def rss_neg_log_likelihood_and_gradient_on_single_chromosome(x, z_scores, chrom_ld_mat_reg_ref, chrom_ld_mat_reg_reg, chrom_anno, sample_size, chrom_num):
	...
	# Compute full covariance
	cov = R_beta2_RT + chrom_ld_mat_reg_reg
	cov = cov.toarray() + np.eye(cov.shape[0])*1e-3
...
def rss_neg_log_likelihood_and_grad(x, trait_name, shared_input_data_dir, trait_specific_input_data_dir, sample_size):
...
#pdb.set_trace()
opti=scipy.optimize.fmin_l_bfgs_b(rss_neg_log_likelihood_and_grad, tau_0, args=(trait_name, shared_input_data_dir, trait_specific_input_data_dir, sample_size), approx_grad=False, iprint=101)
print(opti[0])
