...
for chrom_num in range(1,23):
	...
	for line in f:
		...
		ld_mat_reg_ref = scipy.sparse.load_npz(ld_mat_reg_ref_file)

		ld_mat_reg_reg_file = shared_input_data_dir + 'ld_mat_regression_regression_chr_' + window_size + '_mb_windows_' + window_name + '.npz'
		ld_mat_reg_reg = scipy.sparse.load_npz(ld_mat_reg_reg_file)

		# Fillter regression indices to those we have z-scores for
		ld_mat_reg_ref = ld_mat_reg_ref[regression_snp_indices,:]
		ld_mat_reg_reg = ld_mat_reg_reg[regression_snp_indices,:][:, regression_snp_indices]

		# Create annotation weighted ld scores
		squared_ld_reg_ref = ld_mat_reg_ref.power(2).toarray()
		# Adjusted r-squared
		...
		anno_weighted_ld_scores = scipy.sparse.csr_matrix(squared_ld_reg_ref).dot(anno)

		# Get regression_overlap weights
		squared_ld_reg_reg = ld_mat_reg_reg.power(2).toarray()
		# Adjusted r-squared
		squared_ld_reg_reg[squared_ld_reg_reg!=0.0] = squared_ld_reg_reg[squared_ld_reg_reg!=0.0] - ((1.0 - squared_ld_reg_reg[squared_ld_reg_reg!=0.0])/(487.0))
		regression_overlap_squared_ld = np.sum(squared_ld_reg_reg, axis=0)
		regression_overlap_squared_ld[regression_overlap_squared_ld<1] = 1.0

		# Get chi-squared statistics
		chi_squared_stats = np.square(z_scores)

		chi_sq.append(chi_squared_stats)
		ld_scores.append(anno_weighted_ld_scores)
		regression_ld_scores.append(regression_overlap_squared_ld)
...
regression_ld_scores = np.hstack(regression_ld_scores)
...
regression_ld_scores = regression_ld_scores[valid_indices]
...
# Regression weights
regression_weights = 1.0/(het_var*regression_ld_scores)
